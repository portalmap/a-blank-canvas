...
const getStickerSignedUrl = async (path: string): Promise<string> => {
  const { data } = await supabase.storage.from('stickers').createSignedUrl(path, 3600);
...
export const useStickers = (packId?: string) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (paths.length > 0) {
        const { data: signedData } = await supabase.storage.from('stickers').createSignedUrls(paths, 3600);
        if (signedData) {
