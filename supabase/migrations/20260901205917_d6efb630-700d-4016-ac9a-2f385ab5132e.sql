-- Normalização de nome (minúsculas + acentos)
CREATE OR REPLACE FUNCTION public.norm_status_name(_name text)
RETURNS text
LANGUAGE sql
IMMUTABLE
SET search_path = public
AS $$
  SELECT lower(btrim(translate(coalesce(_name, ''),
    'áàâãäéèêëíìîïóòôõöúùûüçÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇ',
    'aaaaaeeeeiiiiooooouuuucAAAAAEEEEIIIIOOOOOUUUUC')))
$$;

REVOKE EXECUTE ON FUNCTION public.norm_status_name(text) FROM anon;
...
BEGIN
  ...
  LOOP
    -- 0. Unifica duplicidades de vínculo (mesmo item apontado por mais de uma etapa)
    FOR v_dup IN
    ...
    -- 1. Religa etapas sem vínculo, casando por nome normalizado (uma a uma)
    FOR v_item IN
      ...
      ORDER BY sti.order_index, sti.created_at
    LOOP
    ...
    -- 2. Propaga os dados do modelo para as etapas vinculadas
    ...
    -- 3. Cria as etapas do modelo que ainda não existem no local
    ...
    -- 4. Trata as etapas cujo item foi removido do modelo
    FOR v_status IN
    ...
    -- 5. Etapas extras (sem vínculo): mantém no fim se tiver tarefas, remove se estiver vazia
...
-- Propagação automática ao alterar/criar itens do modelo
CREATE OR REPLACE FUNCTION public.trg_status_template_items_resync()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  PERFORM public.resync_template_statuses(NEW.template_id, '{}'::jsonb, '{}'::uuid[]);
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_status_template_items_resync ON public.status_template_items;
CREATE TRIGGER trg_status_template_items_resync
AFTER INSERT OR UPDATE OF name, color, category, is_default, order_index
ON public.status_template_items
FOR EACH ROW
EXECUTE FUNCTION public.trg_status_template_items_resync();

-- Propagação automática quando um local passa a usar (ou troca) o modelo
CREATE OR REPLACE FUNCTION public.trg_scope_status_template_resync()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  IF NEW.status_source = 'template' AND NEW.status_template_id IS NOT NULL THEN
    PERFORM public.resync_template_statuses(NEW.status_template_id, '{}'::jsonb, '{}'::uuid[]);
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_lists_status_template_resync ON public.lists;
CREATE TRIGGER trg_lists_status_template_resync
AFTER UPDATE OF status_source, status_template_id ON public.lists
FOR EACH ROW
WHEN (NEW.status_source IS DISTINCT FROM OLD.status_source
      OR NEW.status_template_id IS DISTINCT FROM OLD.status_template_id)
EXECUTE FUNCTION public.trg_scope_status_template_resync();

DROP TRIGGER IF EXISTS trg_folders_status_template_resync ON public.folders;
CREATE TRIGGER trg_folders_status_template_resync
AFTER UPDATE OF status_source, status_template_id ON public.folders
FOR EACH ROW
WHEN (NEW.status_source IS DISTINCT FROM OLD.status_source
      OR NEW.status_template_id IS DISTINCT FROM OLD.status_template_id)
EXECUTE FUNCTION public.trg_scope_status_template_resync();

DROP TRIGGER IF EXISTS trg_spaces_status_template_resync ON public.spaces;
CREATE TRIGGER trg_spaces_status_template_resync
AFTER UPDATE OF status_source, status_template_id ON public.spaces
FOR EACH ROW
WHEN (NEW.status_source IS DISTINCT FROM OLD.status_source
      OR NEW.status_template_id IS DISTINCT FROM OLD.status_template_id)
EXECUTE FUNCTION public.trg_scope_status_template_resync();